...
int main( int argc, char** argv )
{
	...
	{
		printf( "\n" );
		printf( "Usage: %s [bond_length=X] [leaflet_separation=X] [lipid=t1,t1,...[:b1,b2,...][:a1,a2,...], ...] [sphere=monolayer|bilayer:MOLSTRING:outer_r:APL, ...] [plane=bilayer|monolayer:MOLSTRING:N:APL, ...]\n", argv[0] );
		printf( "\n" );
		printf( "Where:\n" );
		printf( "\n" );
		printf( "MOLSTRING : comma-separated lists of lipid types and relative proportions, separated by question mark.\n" );
		printf( "\n" );
		printf( "Examples:\n" );
		printf( "\n" );
		printf( "%s lipid=1,2,3 sphere=bilayer:1:100:70\n", argv[0] );
		printf( "%s bond_length=7.5 leaflet_separation=7.5 lipid=1,2,3,3,3 lipid=1,2,3 sphere=bilayer:1,2?1,1:100:70\n", argv[0] );
		printf( "%s lipid=1,2,3,3,3 lipid=1,2,3 sphere=bilayer:1,2?10,12:100:70\n", argv[0] );
		printf( "\n" );
		printf( "Notes:\n" );
		printf( "\n" );
		printf( "Molecule types are UNIT BASED and correspond to the order in which lipid definitions occurred.\n" );
