/*
	...
	This differs from the FluctuationSpectrum.cpp program as it uses the LAMMPS
	config class rather than the LAMMPS trajectory frame class. It's here in case
	I decide to finally delete the trajectory frame class, as that's less general
	vs the config class (but it can be faster, and it can use less memory).
*/
...
struct Data
{
	...
	template<typename T>
	static int to_int( const std::string& what, const std::string& str, T& val )
	{
...
int main( int argc, char** argv )
{
	...
	//
	// LAMMPS trajectory frame & flattened arrays
	//
	...
	// Surface point info
	//
	std::vector<int> head_idx, tail_idx, upper_idx, lower_idx, combined_idx, filtered_idx;
	std::vector<double> surf_x, surf_y, surf_z;
	...
	while( true )
	{
		...
		//
		{
			int result = LAMMPS::LoadTrajectory( in_traj, config );
			if( result == -1 || config.molecules.size() < 3 )
			{
		...
		//
		double minx = config.bounds.minx * data.length_scale;
		double miny = config.bounds.miny * data.length_scale;
		double minz = config.bounds.minz * data.length_scale;

		double maxx = config.bounds.maxx * data.length_scale;
		double maxy = config.bounds.maxy * data.length_scale;
		double maxz = config.bounds.maxz * data.length_scale;
		...
		for( const auto& m : config.molecules )
		{
			...
			{
				x_vec.push_back( a.x * data.length_scale );
				y_vec.push_back( a.y * data.length_scale );
				z_vec.push_back( a.z * data.length_scale );
		...
		//
		{
			...
			{
				int atom_type = atom_types[i];
				double z = z_vec[i];

				// Detect heads/tails
				if( data.IsHead(atom_type) == true ) head_idx.push_back( i );
				if( data.IsTail(atom_type) == true ) tail_idx.push_back( i );
		...
		if( data.filter_size > 0.0 )
		{
			...
			printf( "%d/%d heads passed filtering.\n", result, (int)head_idx.size() );
			head_idx = filtered_idx;
			...
			printf( "%d/%d tails passed filtering.\n", result, (int)tail_idx.size() );
			tail_idx = filtered_idx;
		}
...
//				heads, tails,
			...
			combined_idx = upper_idx;
			for( auto& i : lower_idx ) combined_idx.push_back( i );
		}
		...
		//
		{
			...
			{
				for( auto i : upper_idx )
				{
					surf_x.push_back( x_vec[i] );
					surf_y.push_back( y_vec[i] );
					surf_z.push_back( z_vec[i] );
				}
			}
			else if( data.which_monolayer == "lower" )
			{
				for( auto i : lower_idx )
				{
					surf_x.push_back( x_vec[i] );
					surf_y.push_back( y_vec[i] );
					surf_z.push_back( z_vec[i] );
			...
			else if( data.which_monolayer == "midplane" )
			{
				...
				for( int xi=0; xi<N_gx; xi++ )
				{
					...
					for( int yi=0; yi<N_gy; yi++ )
					{
						...
						double z_ = data.membrane_grid.midplane[index]; // midplane generated from scaled particle coords
			...
			//
			{
				...
				for( size_t i=0; i<surf_z.size(); i++ )
				{
					double x_ = surf_x[i];
					double y_ = surf_y[i];
					double z_ = surf_z[i];
				...
				average_z /= surf_z.size();
				for( auto& zi : surf_z ) zi -= average_z;
			}
			...
			int result = data.Process( surf_x, surf_y, surf_z, Lx, Ly, prefactor );
			if( result == -1 )
			{
		...
		//
		{
			printf( "Frame %d, dims %g,%g,%g -> %g,%g,%g : %g,%g,%g, %d heads %d tails, %d upper lipids %d lower\n",
				...
				maxx, maxy, maxz,
				Lx, Ly, Lz,
				(int)head_idx.size(), (int)tail_idx.size(),
				(int)lower_idx.size(), (int)upper_idx.size() );
		}
		...
		//
		{
			double Nm = 0.5*(lower_idx.size()+upper_idx.size()); // lipids per monolayer
